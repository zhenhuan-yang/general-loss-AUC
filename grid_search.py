...
beta_dict = {0: np.array([11, 110, 495, 1320, 2310, 2772, 2310, 1320, 495, 110, 11]),
             ...
             4: np.array([2310, 13860, 34650, 46200, 34650, 13860, 2310]),
             5: np.array([2772, 13860, 27720, 27720, 13860, 2772]),
             6: np.array([2310, 9240, 13860, 9240, 2310]), 7: np.array([1320, 3960, 3960, 1320]),
             8: np.array([495, 990, 495]),
...
def SOLAM(t, loss, index, L, R1, R2, lam, theta, c, wt, at, bt, alphat):
    ...
    wt = wt - peta * (gradwt * LABELS[index] * FEATURES[index] / (
                2 * (N + 1)) + lam * wt)  # step size as 1/t gradient descent
...
def demo(loss, alg, train_list, test_list, L, R1, R2, gamma=0.01, lam=0, theta=.5, c=1, complete=False):
    ...
    for t in range(1, T + 1):
        ...
        W = ((t-1)*W + WT)/t
        try:
            roc_auc[t-1] = roc_auc_score(LABELS[test_list], np.dot(FEATURES[test_list], W))
